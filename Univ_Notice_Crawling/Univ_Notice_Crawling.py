...
def snu_career_notice():
    ...
    driver.get(url)
    driver.implicitly_wait(2) # 2초간 정지
    ...
    for _ in range(10):
        ...
        for i in range(len(article_raw)-12):
            if article_raw[i].text != "":
                if article_raw[i].text in article_list:
                    break_point = 1
                    break
                article_list.append(article_raw[i].text)
                from_list.append("서울대학교 공개채용")
        if break_point == 1:
            break
        ...
        for i in range(len(date_raw)):
            if date_raw[i].text != "":
                if date_raw[i].text[0] == "마":
                    date_list.append(date_raw[i].text)
